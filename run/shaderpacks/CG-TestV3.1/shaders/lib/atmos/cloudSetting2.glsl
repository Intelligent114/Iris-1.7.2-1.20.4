#if CLOUD_SIZE_MULT_FIX != 100
    #define CLOUD_SIZE_MULT_FIX_M CLOUD_SIZE_MULT_FIX * 0.01
#endif
...
#else
    const float cloudStretch = cloudStretchRaw / float(CLOUD_SIZE_MULT_FIX_M);
#endif
...
float GetCloudNoise(vec3 tracePos, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY) {
    ...
    #endif
    #if CLOUD_SIZE_MULT_FIX != 100
        tracePosM *= CLOUD_SIZE_MULT_FIX_M;
        wind *= CLOUD_SIZE_MULT_FIX_M;
    #endif
    ...
    noiseMult *= CLOUD_BASE_ADD
                ...
                + CLOUD_RAIN_FIX * rainFactor;
    noise *= noiseMult * CLOUD_AMOUNT_FIX;
...
vec4 GetVolumetricClouds(int cloudAltitude, float distanceThreshold, inout float cloudLinearDepth, float skyFade, float skyMult0, vec3 cameraPos, vec3 nPlayerPos, float lViewPosM, float VdotS, float VdotU, float dither) {
    ...
    #if CLOUD_SIZE_MULT_FIX > 100
        stepMult = stepMult / sqrt(float(CLOUD_SIZE_MULT_FIX_M));
    #endif

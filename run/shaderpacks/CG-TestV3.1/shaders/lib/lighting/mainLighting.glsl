...
#include "/lib/lighting/ggx.glsl"
...
#if defined CLOUDS_SETTING1 && defined CLOUD_SHADOWS
    #include "/lib/atmos/cloudCoord.glsl"
#endif
    ...
    #include "/lib/others/pixelation.glsl"
#endif
...
void DoLighting(inout vec4 color, inout vec3 shadowMult, vec3 playerPos, vec3 viewPos, float lViewPos, vec3 geoNormal, vec3 normalM, float dither,
        ...
        vec2 pixelationOffset = ComputeTexelOffset(tex, texCoord);
            ...
            vec3 playerPosPixelated = TexelSnap(playerPos, pixelationOffset);
                ...
                if (entityId == 50076) {
                    playerPosPixelated.y += 0.38;
                }
            #endif
            #ifdef GBUFFERS_TERRAIN
                if (subsurfaceMode == 1) {
                    playerPosPixelated.y += 0.05;
                }
            #endif
        #endif
    ...
    vec3 nViewPos = normalize(viewPos);
        ...
        if (shadowMult.r > 0.00001) {
                ...
                vec3 worldPos = playerPos + cameraPosition;
                    ...
                    csPos.x += syncedTime;
                    csPos *= 0.000002 * CLOUD_SIZE_MULT_FIX;
    ...
    if (!noSmoothLighting) {
        ...
        lightmapXM = pow(lightmapXMSteep + lightmapXMCalm, 2.5);
    } else lightmapXM = pow2(lightmap.x) * lightmap.x * 12.0;
    ...
    vec3 blockLighting = lightmapXM * blocklightCol;
        ...
        float heldLight2 = heldBlockLightValue2;
            ...
            if (heldItemId == 45032) heldLight = 15; if (heldItemId2 == 45032) heldLight2 = 15;
        #else
            ...
            if (heldItemId == 45032) { heldLightCol = lavaSpecialLightColor; heldLight = 15; }
        ...
        heldLight = pow2(pow2(heldLight * 0.3 / lViewPosL));
        heldLight2 = pow2(pow2(heldLight2 * 0.3 / lViewPosL));
    ...
    #ifdef OVERWORLD
        float sunWhiteMix = 0.25;
        lightColorM = mix(lightColorM, vec3(1.0), sunWhiteMix);
    #endif

    #ifdef END
        float moonWhiteMix = 0.25;
        lightColorM = mix(lightColorM, vec3(1.0), moonWhiteMix);
    #endif
